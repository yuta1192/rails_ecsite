...
- [ ] 在庫の変動確認
  - 商品がどれだけ在庫が変化したかを見れる
  - １日、１週間、1ヶ月、１年単位
  - １日の在庫減少数 = ProductPurchaseの特定商品のnum数のsum（合計）をcreated_atで集計を取ればいいかも増加分はstock_updatesに入れることにした
- [ ] 一括追加
  - CSVで一括(mada)
  - 選択したものを一括で選択した個数在庫を追加(ok)
- [ ] 管理者に品切れメール
  - 在庫切れをした場合メールを管理者に送る
- [x] 在庫なしのみ発注機能
  - 在庫なしのみ一括で発注
...
- [x] 詳細、一覧
  - 受注内容の詳細、一覧を作るx
- [x] 終了
  - 既に受注処理が終わっている一覧、詳細(出荷に移す)->チェックがindexでできるようにしたい、management.html.slimを参照して
- [x] 受注状況CSV
  - 印刷用ダウンロード
  - インポート
- [x] order_idで検索できる
...
- [ ] トータルピッキングリスト
  - [ ] 商品ごとに出荷数量を集計して印刷用のファイル作成
   - 商品名、合計購入個数
   - エクスポート -> できてない
   - 商品検索、ソート（購入数順）、オーダー（購入数有りか無しか） -> ソートできてない
  - [x] 商品の詳細
    - 購入者情報
    - 出荷合計
    - 商品の詳細
